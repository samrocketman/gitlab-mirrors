...
def createproject(pname):
    ...
    project_for_transfer = find_project(name=pname)
    if needs_transfer(gitlab_user, gitlab_namespace, project_for_transfer):
        project_for_transfer = transfer_project(project_for_transfer, found_group)
...
def needs_transfer(user, groupname, project):
    ...
    if groupname:
        ...
        namespace = user
    if isinstance(project.namespace, gitlab.v4.objects.Group):
    ...
    if found_project:
        if needs_transfer(gitlab_user, gitlab_namespace, found_project):
            ...
            if not found_project:
                printErr(
                    "There was a problem transferring {group}/{project}.  Did you give {user} user Admin rights in gitlab?".format(
        ...
        if not found_project:
            printErr(
                "There was a problem creating {group}/{project}.  Did you give {user} user Admin rights in gitlab?".format(
                    group=gitlab_namespace, project=project_name, user=gitlab_user
                )
            )
            sys.exit(1)
